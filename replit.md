...
*   **Prontuário Multiprofissional:** Patient detail reorganized into 19 tabs by professional category: Resumo Clínico, Atendimento, Sinais Vitais, Linha do Tempo, Evol. Médica, Prescrição, Sol. Exames, Enfermagem, SAE, Técnico Enf., Serviço Social, Nutrição, Farmácia, Exames, Laboratório, Regulação/NIR, Transferência, SINAN, Dispositivos.
*   **Regulação/NIR:** `patient_nir_entries` table + GET/POST `/api/patients/:id/nir` — records tipo, conteudo, status_vaga, prioridade, destino per patient. Frontend: `patient-nir-tab.tsx`.
*   **Linha do Tempo:** GET `/api/patients/:id/timeline` aggregates evolutions, prescriptions, vitals, exam-requests, transfers, pharmacy, NIR entries into a chronological event list. Frontend: `patient-timeline-tab.tsx`.
*   **Resumo Clínico tab:** `patient-resumo-tab.tsx` shows patient admission data, latest vitals, active prescriptions/exams count, and last evolution snippet in a single-glance card layout.
