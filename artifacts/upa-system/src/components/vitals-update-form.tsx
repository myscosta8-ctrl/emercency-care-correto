...
const schema = z.object({
  ...
  hr:          z.coerce.number().min(0).default(0),
  rr:          z.coerce.number().min(0).default(0),
  spo2:        z.coerce.number().min(0).max(100).default(0),
  temp:        z.coerce.number().min(0).default(0),
...
function SoapSection({ letter, title, colorClass, children }: {
  letter: string; title: string; colorClass: string; children: React.ReactNode;
}) {
  return (
...
export function VitalsUpdateForm({ patient, onSuccess, onCancel }: VitalsUpdateFormProps) {
  ...
  function onSubmit(data: FormValues) {
    const bp = data.systolicBp > 0 || data.diastolicBp > 0
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)}>
        <div className="mb-5 pb-4 border-b border-border/50">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Paciente: <strong className="text-foreground">{patient.full_name}</strong></p>
        </div>
        ...
        <div className="space-y-6 max-h-[60vh] overflow-y-auto pr-1">
          ...
          <SoapSection letter="S" title="Subjetivo" colorClass="border-blue-500/70">
            <FormField control={form.control} name="subjective" render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <textarea {...field} rows={2} placeholder="Dor, dispneia, náusea, tontura..."
            ...
            <div className="space-y-4">
              ...
              <FormField control={form.control} name="objective" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <textarea {...field} rows={2} placeholder="Ex.: edema ++ em MMII, ausculta com murmúrio vesicular reduzido..."
                      className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring resize-none"
                    />
                  </FormControl>
                  <FormMessage />
              ...
              <div>
                ...
                <ToggleGroup
                  options={[{ value: "bom", label: "Bom" }, { value: "regular", label: "Regular" }, { value: "grave", label: "Grave" }]}
              ...
              <div>
                ...
                <ToggleGroup
                  options={[
                    { value: "lucido", label: "Lúcido" }, { value: "sonolento", label: "Sonolento" },
                    { value: "torporoso", label: "Torporoso" }, { value: "inconsciente", label: "Inconsciente" },
                  ]}
                ...
                <label className="text-xs text-muted-foreground block mb-2">
                  ...
                  {painScale > 0 && (
                    <span className={cn("font-bold",
                      painScale <= 3 ? "text-triage-green" : painScale <= 6 ? "text-triage-yellow" : "text-triage-red"
                    )}>{painScale}/10</span>
                  )}
                </label>
                <div className="flex gap-1 flex-wrap">
                  {Array.from({ length: 11 }, (_, i) => (
                    <button key={i} type="button" onClick={() => form.setValue("painScale", i)}
                      className={cn(
          ...
          <SoapSection letter="A" title="Avaliação" colorClass="border-orange-500/70">
            <FormField control={form.control} name="assessment" render={({ field }) => (
              <FormItem>
                <FormControl>
                  <textarea {...field} rows={2} placeholder="Paciente em condição estável/instável/crítica, com quadro de..."
                    className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring resize-none"
                  />
                </FormControl>
                <FormMessage />
          ...
          <div className="space-y-3 pt-2 border-t border-border/50">
            <FormField control={form.control} name="responsible" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Input placeholder="Nome do Técnico(a) ou Enfermeiro(a)" {...field} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
            <FormField control={form.control} name="note" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Input placeholder="Intercorrências, procedimentos realizados..." {...field} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
          </div>
        ...
        <div className="flex justify-end gap-2 pt-4 mt-4 border-t border-border/50">
          <Button type="button" variant="outline" onClick={onCancel} disabled={addVitals.isPending}>Cancelar</Button>
